...
def main():
    ...
    if len(sys.argv) > 1 and sys.argv[1] in ["--presets", "-p", "--list"]:
        ...
        print("\n💡 Diferenças dos presets:")
        print("   - maximum_compression: Compressão máxima mantendo resolução original")
        print("   - ultra_compression_1080p: Reduz para 1080p + compressão máxima")
        print("   - ultra_compression_720p: Reduz para 720p + compressão máxima")
        print("\nOu via variável de ambiente:")
        print("  set PRESET_VIDEO=maximum_compression")
        print("  python otimizador-compressor-video.py")
        print("\n🔧 Controle de Recursos (variáveis de ambiente):")
        print("  FFMPEG_THREADS=4          # Número de threads (padrão: 50% dos cores, máx 8)")
